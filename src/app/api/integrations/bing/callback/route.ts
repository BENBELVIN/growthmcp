...
import { exchangeCodeForTokens } from "@/lib/bing/oauth";
import { BING_OAUTH_STATE_COOKIE } from "@/lib/bing/env";
import { getSiteOrigin } from "@/lib/gsc/env";
import { listBingSites } from "@/lib/bing/client";
...
export async function GET(request: NextRequest) {
  ...
  const cookieState = request.cookies.get(BING_OAUTH_STATE_COOKIE)?.value;
  ...
  try {
    ...
    if (website) {
      try {
        const sites = await listBingSites(tokens.access_token);
        const { preferred } = preferBingProperty(website.url, sites);
        if (preferred) {
          await supabase
            .from("bing_connections")
            .update({
