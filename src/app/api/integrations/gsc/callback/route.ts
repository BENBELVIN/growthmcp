...
export async function GET(request: NextRequest) {
  ...
  const fail = (msg: string) =>
    NextResponse.redirect(
      new URL(`/dashboard/settings?gsc_error=${encodeURIComponent(msg)}`, origin)
    );
  ...
  try {
      ...
      try {
        ...
        if (preferred) {
          ...
          const res = NextResponse.redirect(
            new URL("/dashboard/settings?gsc=connected", origin)
          );
    ...
    const res = NextResponse.redirect(
      new URL(
        `/dashboard/settings/integrations/gsc?websiteId=${payload.websiteId}`,
        origin
      )
    );
    res.cookies.delete(GSC_OAUTH_STATE_COOKIE);
