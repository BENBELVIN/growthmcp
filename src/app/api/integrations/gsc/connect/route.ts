...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.redirect(
      new URL(
        `/dashboard/settings?gsc_error=${encodeURIComponent(msg)}`,
        request.url
      )
    );
  }
  ...
  if (!websiteId) {
    return NextResponse.redirect(
      new URL("/dashboard/settings?gsc_error=missing_website", request.url)
    );
  }
  ...
  if (!website) {
    return NextResponse.redirect(
      new URL("/dashboard/settings?gsc_error=website_not_found", request.url)
    );
  }
