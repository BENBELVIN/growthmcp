    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-28 animate-pulse rounded-3xl bg-muted/20" />
        ))}
      </div>
    </div>
  );
...
function directionTone(direction: string) {
  ...
  if (direction === "falling") return "text-rose-600";
  ...
  if (!data || (data.trends.length === 0 && !loading)) {
    return (
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Discover rising search demand for your niche
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="rounded-3xl border border-dashed border-border bg-card px-6 py-12 shadow-sm">
          <TrendingUp className="size-5 text-muted-foreground" />
          <h4 className="mt-4 text-base font-semibold tracking-tight">
            No Trends data yet
          </h4>
          <p className="mt-2 max-w-lg text-sm text-muted-foreground">
            Sync pulls interest for your top Search Console queries and surfaces
            keyword opportunities — topics to create content around.
          </p>
    ...
    <div className={cn("space-y-8", loading && "opacity-70")}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {data.trends.length} keyword
              {data.trends.length === 1 ? "" : "s"} tracked
              {data.opportunities.length > 0 &&
                ` · ${data.opportunities.length} content opportunities`}
            </p>
          </div>
        </div>
        <Button
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Actionable recommendations from Trends and Search Console
          </p>
        </div>
        {data.opportunities.length === 0 ? (
          <p className="rounded-3xl border border-border bg-card px-5 py-6 text-sm text-muted-foreground shadow-sm">
            No open keyword opportunities right now. Refresh after Search Console
            has more query data.
          </p>
        ) : (
            ...
            {data.opportunities.map((opp) => (
              ...
              >
                ...
                <Button size="sm" className="h-9 shrink-0 rounded-full" asChild>
                  <Link href="/dashboard/content-rankings">
                    Check rankings
      ...
      <section className="space-y-3">
        ...
        <div className="h-64 w-full rounded-3xl border border-border bg-card p-3 shadow-sm sm:p-4">
