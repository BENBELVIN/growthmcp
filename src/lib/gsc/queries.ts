...
export async function getGscConnectionForWebsite(
  websiteId: string
  ...
  if (error) {
    ...
    if (error.message.includes("gsc_connections")) return null;
...
export async function getGscConnectionSecret(websiteId: string) {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("gsc_connections")
    .select(
